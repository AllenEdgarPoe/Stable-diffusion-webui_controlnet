...
class Script(scripts.Script):
    ...
    def uigroup(self, is_img2img):
        ...
        with gr.Row():
            gr.HTML(value='<p>Sketch 를 하려면 아래 Create blank canvas 를 누르고 좀만 기다리세요. <br> 모든 그림은 제 컴퓨터에 저장됩니다. <br ></p>')
        ...
        with gr.Row():
            module = gr.Dropdown(list(self.preprocessor.keys()), label=f"Preprocessor", value="scribble")
            model = gr.Dropdown(list(cn_models.keys()), label=f"Model", value="control_sd15_scribble [fef5e48e]")
        ...
        def run_annotator(image, module, pres, pthr_a, pthr_b):
            ...
            if not ((image['mask'][:, :, 0]==0).all() or (image['mask'][:, :, 0]==255).all()):
                img = HWC3(image['mask'][:,:,0])+np.invert(HWC3(image['image']))
        ...
        for idx, contents in enumerate(control_groups):
            ...
            if is_img2img_batch_tab and hasattr(p, "image_control") and p.image_control is not None:
                ...
                #     scribble_mode = True
                    ...
                    # input_image = input_image + HWC3(image['mask'][:, :, 0])
                    # input_image = input_image.clip(0, 255)
                    input_image = HWC3(image['mask'][:,:,0])+np.invert(HWC3(image['image']))
            else:
